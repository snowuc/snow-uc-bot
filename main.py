...
def callback_query(call):
    ...
    if call.data == "buy_menu":
        ...
        packs = [("60 UC", "buy_60"), ("120 UC", "buy_120"), ("180 UC", "buy_180"), ("325 UC", "buy_325"), 
                 ...
                 ("3850 UC", "buy_3850"), ("4510 UC", "buy_4510"), ("5650 UC", "buy_5650"), ("8100 UC", "buy_8100"), 
                 ("9900 UC", "buy_9900"), ("11950 UC", "buy_11950"), ("16200 UC", "buy_16200")]
        markup.add(*[types.InlineKeyboardButton(p[0], callback_data=p[1]) for p in packs])
        bot.edit_message_text(text="💎 Выберите пак:", chat_id=chat_id, message_id=call.message.message_id, reply_markup=markup)
    elif call.data.startswith("buy_"):
        user_data[chat_id] = {"pack": call.data.split("_")[1]}
        bot.edit_message_text(text="Введите ваш игровой ID (начинается на 5):", chat_id=chat_id, message_id=call.message.message_id)
    elif call.data == "confirm_yes":
        bot.edit_message_text(text="💳 Карта: <code>5168 7500 0000 0000</code>. Пришлите чек.", chat_id=chat_id, message_id=call.message.message_id, parse_mode="HTML")
    elif call.data == "confirm_no":
        if chat_id in user_data: del user_data[chat_id]
        bot.edit_message_text(text="❌ Отменено. Напишите /start", chat_id=chat_id, message_id=call.message.message_id)
...
def handle_id(message):
    if message.text.startswith('5'):
        ...
        bot.send_message(message.chat.id, f"ID: {message.text}. Верно?", reply_markup=markup)
    else:
...
def handle_photo(message):
    if message.chat.id in user_data:
        bot.send_photo(ADMIN_ID, message.photo[-1].file_id, caption=f"🔔 Заказ!\nОт: {message.chat.id}\nID: {user_data[message.chat.id]['id']}\nПак: {user_data[message.chat.id]['pack']}")
        bot.reply_to(message, "✅ Чек отправлен админу.")
